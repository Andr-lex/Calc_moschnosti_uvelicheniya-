...
st.set_page_config(page_title="Триада-ТКО: Калькулятор Формы 15", layout="wide")

st.title("📊 Интерактивный калькулятор потребности в мощностях (ФОРМА № 15)")
st.subheader("Разработано для Директора по развитию ОП Айт Ольги Андреевны")

st.markdown("""
Этот калькулятор предназначен для верификации заявок на расширение площадей обособленных подразделений (ОП) в рамках Гособоронзаказа (ГОЗ).
...
with col2:
    ...
    if diff < 0:
        ...
        st.markdown(f"""
        **Рекомендация Ольге Айт:** Собственник навязывает вам лишние `{abs(diff):.2f} кв.м.`, которые не подтверждены технологическим расчетом. 
        Принятие этих затрат на Гособоронзаказ (ГОЗ) приведет к завышению себестоимости и рискам уголовной ответственности по закону № 275-ФЗ!
